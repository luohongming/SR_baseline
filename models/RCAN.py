...
class RCANModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        args = {'scale': opt.sr_factor, 'n_resgroups': 10, 'n_feats': 64, 'n_resblocks': 20, 'res_scale': 1,
                'n_colors': 3,
                'reduction': 16, 'rgb_range': opt.rgb_range}
        self.model = RCAN(args).to(self.device)
        self.criterion = nn.L1Loss()
        self.model = nn.DataParallel(self.model, opt.gpu_ids)
        # self.criterion = self.criterion.to(self.device)
        self.optimizer = optim.Adam(self.model.parameters(), lr=opt.lr)
        self.scheduler = lr_scheduler.StepLR(self.optimizer, step_size=430, gamma=0.5)
